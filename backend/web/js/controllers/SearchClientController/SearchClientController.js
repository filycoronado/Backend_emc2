...
//        }
        ...
        $scope.valueLabel = "";
        $scope.CompareDate = function (date, effective, fechamed, exp, credit_value, payment_date_credit) {
            ...
            //console.log(paydate);
            //console.log(Pending);
            //console.log(date_creditPending);

            if (credit_value > 0) {
                var date_creditPending = new Date(payment_date_credit);
                var date_credit = new Date(payment_date_credit).toISOString().substring(0, 10);
                date_creditPending.setDate(date_creditPending.getDate() + 7);
                date_creditPending = date_creditPending.toISOString().substring(0, 10);

                if (date > date_credit && date <= date_creditPending) {
                    $scope.valueLabel = 2;
                    return "Pending Cancellation";
                } else if (date > date_creditPending) {
                    $scope.valueLabel = 3;
                    return "Cancelled";
                } else if (date < date_credit) {
                    $scope.valueLabel = 1;
                    return "Active";
                } else {
                    return "nw";
