app.controller('AddCoveragesController2', ['$scope', '$rootScope', '$http', '$routeParams', '$window', '$location', '$filter', function ($scope, $rootScope, $http, $routeParams, $window, $location, $filter) {
        $scope.client = {};
        $scope.display = true;
        ...
        $scope.selectServicePlan = function () {
            if ($scope.client.plan) {
                $scope.selectedPlan = $filter('filter')($scope.planPeriods, {'value': $scope.client.plan, 'serviceId': $scope.client.service})[0];
            }
            //forzar glass en todos los vehiculos previamente agregados
        ...
        $scope.calculo = function (s) {
            ...
            $scope.NewValueMember = calculo;
            return calculo;
        }

        $scope.RemoveVehicle = function (v) {
            if ($scope.vehicles.length > 1) {
                var index = $scope.vehicles.indexOf(v);
                $scope.vehicles.splice(index, 1);
            } else {
                alert("A vehicle is required as a minimum.");
            }
            $scope.calculo();
        }
...
//            }
        ...
        $http.post('index.php?r=api/get_client_by_id', {id_client: $scope.id_client}).then(function success(response) {
            ...
            $scope.old_value = $scope.client.total;
        ...
        $scope.SaveCoverage = function () {
            $http.post('index.php?r=api/updatecoverages', {old_value: $scope.old_value, modelClient: $scope.client, Gtotal: $scope.NewValueMember, vehicles: $scope.vehicles, id_client: $scope.id_client, cardInfo: $scope.cardInfo}).then(function success(response) {
                alert("Successfully");
        ...
        $scope.Decode = function () {
            ...
            $http.post('index.php?r=api/vin_decode', {vin: $scope.vehicle.vin}).then(
                    function success(response) {

                        $scope.getV = response.data;
                        $scope.vehicle.model = $scope.getV.model;
                        $scope.vehicle.year = $scope.getV.year;
                        $scope.vehicle.make = $scope.getV.make;
