...
class ApiController extends Controller {
    ...
    function actionUsers_by_agency() {
        $userinlog = Yii::$app->user->identity;

        $user = (new Query())->select('*')->from('users')
                ->where("habilitado=1")
                ->andWhere("id=" . $userinlog->id)
                ->all();
        if ($user != null) {
            return $user;
        }
    ...
    function actionAgencies_by_owner() {
        ...
        $agency = Agency::find()->where("id=" . $user->agencia)->all();
        return $agency;
    }
    ...
    function actionGet_tikets_by_date() {
        ...
        $query = new Query;
        $query->select('clients.Dpayment as membership_status,clients.id_agencia as ag, clients.name as name , clients.apellido as lastname, clients.plan as plan, tickets.total as tot, tickets.*'
                )
                ->from('clients')
...
//$this->DoPaymentCash($modelClient, "INVOICE", Yii::$app->user->identity->user, 0, 0, "CASH", true);
    ...
    function RewriteMembership($id_client, $label) {
        $modelClient = new Client();
    ...
    function RenewMembership($id_client, $label) {
        $modelClient = new Client();
        $modelClient = Client::findOne($id_client);
        $plan = $modelClient->plan;
        $modelClient->Dpayment = $label;
    ...
    function updateDatesInPeriod($id_client, $case, $label) {
        $modelClient = new Client();
        $modelClient = Client::findOne($id_client);
        $plan = $modelClient->plan;
...
//        }
    ...
    public function actionGet_change_log() {
        ...
        )->from('clients_changelog')->join('LEFT OUTER JOIN', 'users', 'users.id=clients_changelog.id_user')->all();
    ...
    public function actionSave_claim() {
        ...
        $vehicle = Vehicles::find($vehicleID)->one();
    ...
    public function actionSearchclients() {
        ...
        if ($idClient != null) {
            ...
            $clients = Client::find()
                    ...
                    ->andWhere("apellido like '%" . $LastName . "%'")
                    ->andwhere("id_users='" . $user->user . "'")
                    ->all();
        }
        $response;
    ...
    public function actionMember_card($id) {
        ...
        if ($client) {
            ...
            $pdf->Image(Yii::getAlias('@frontend') . '/web/images/logo.png', 10, 1, 35, 38, 'png', 'http://AmericaDriverClub.com/');
            $pdf->SetXY(50, 15);
            $pdf->Cell(40, 7, "America Drivers Club", 0, 1, ' R ');
            $pdf->SetXY(50, 10);
            // $pdf->Cell(40,7,'Report'.$f1."/".$f2,0, 1 , ' R ');
    ...
    public function CreateSalesReport($tickets) {
        ...
        if ($tickets != null) {
            ...
            for ($i = 0; $i < count($tickets); $i++) {
                ...
                $pdf->Cell(35, 5, $tickets[$i]['ag'], 1, 0, 'C');
                $pdf->Cell(35, 5, $tickets[$i]['name'] . " " . $tickets[$i]['lastname'], 1, 0, 'C');
                $pdf->Cell(35, 5, $tickets[$i]['payment'], 1, 0, 'C');
                $pdf->Cell(35, 5, $tickets[$i]['label_status'], 1, 0, 'C');
                $pdf->Cell(35, 5, $tickets[$i]['total'], 1, 0, 'C');
                $pdf->Ln();
            }
